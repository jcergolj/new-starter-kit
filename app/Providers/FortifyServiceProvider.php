...
class FortifyServiceProvider extends ServiceProvider
{
    ...
    private function configureRedirects(): void
    {
        $this->app->singleton(RegisterResponseContract::class, function ($app) {
            return new class($app->make(SubdomainUrlBuilder::class)) implements RegisterResponseContract
            {
                public function __construct(private SubdomainUrlBuilder $urlBuilder) {}

                public function toResponse($request)
                {
                    $username = $request->input('username');
                    $loginUrl = $this->urlBuilder->build($username, '/login');
